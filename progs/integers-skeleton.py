...
# z を自然数の組 (m,n) で表す．0 は (0,0)=(m,m) とし， N と -N の要素はそれぞれ (n,0) か (0,n) で表す．
# 云い方を変えて整数を m,n を自然数，k を整数として m-n=k とする．よって m=n+k となる．この直線の m 軸との交点はちょうど
# m 軸を数直線と見なしたときの数直線上の整数になっている．
#
# m+n'=m'+n となるような組 (m,n), (m',n') 全体を [m,n] で表す． [m,n] を整数 k と表す．[m,n]=[m,0]+[0,n]=m-n である．
# よって，m-n とすれば普段見慣れている整数になる．
#
...
# ...
#(4)
# inverse_on_int は z の逆元を求める関数．逆元とは，下方に関する単位元 (0,0) として，(a,b)+(a',b')=(a',b')+(a,b)=(0,0) となるような (a',b') のことを (a,b) の加法に関する逆元という．
# 逆元がわかれば減算は逆元を足せばいい．
